...
### Creator-reference mode

If the user names a creator reference for this build — "build this like @handle", "use @handle's style", "blend @h1 + @h2" — **before brainstorming scenes**, read the matching style fingerprint(s):

1. `_reference/creator-library/<handle>/style-distilled.md` if it exists (auto-written at ≥3 analyses).
2. Otherwise read each of `_reference/creator-library/<handle>/videos/*/analysis.md` directly.

Apply their signature moves (hook structure, caption style, pacing cadence, transition rotation, scene archetypes, face treatment) on top of the May Shorts 19 playbook below. The creator fingerprint **overrides** `MOTION_PHILOSOPHY.md` defaults for these dimensions, but stays subordinate to `PREFERENCES.md` — if a creator's signature move conflicts with a captured preference, the preference wins.

If the named handle doesn't have a folder in `_reference/creator-library/`, stop and tell the user it hasn't been studied yet. Suggest `/study-creator <url>` on 1–3 of their videos before the build.
...
- Active word: scale-1.08 pop + color change to your brand accent (`var(--brand-accent)`)
...
A flat dark fill alone is a placeholder, not a design. For a techy/control-room aesthetic, build a layered background stack: HUD grid masked to a vignette + circuit traces + pulse nodes + a scan beam + a telemetry ticker + corner mono labels.
...
## Face-mode & scene lessons

Distilled from four concrete problems early short-form renders ran into and what fixed them. Apply these on every new short.

### 1. Slam/stamp timing — reveal logic beats word-sync

A SLAM/STAMP overlay (KILLED, DEAD, STOP, etc.) lands AFTER its target text is fully visible, **not** during the spoken word. In one early build, two products were pitched as opponents — the KILLED stamp fired at local 0.46s while the second product's name didn't appear until 0.66s, so viewers saw "X … KILLED" with no visible target. The joke collapsed.
...
Default `BOTTOM = { x: 0, y: 1136, scale: 0.5625 }` (exact horizontal fit for a 1920×1080 source) leaves empty studio background flanking the speaker if they occupy <70% of the source frame. This is a common starting default — and it leaves visible dead space left and right of the speaker in every BOTTOM scene when the source framing isn't tight.
...
For scenes 3–5 of a 15–20s short (the "middle grind" where attention drops the hardest), lean on visuals that read as information — bar races, stat grids with counting numbers, heatmaps, sparklines, flowcharts, dashboard chrome with telemetry ticking, pain-point grids that flash red in sequence. One early build's scene 4 was a radar-rings + terminal-chip + sparks combo — functional but decorative, and it read as "bland." Replacing it with a 3×3 pain-point grid lighting up red-orange in sequence + a sparkline stroke-drawing with a YOU-ARE-HERE marker + the payoff slam — same time budget — landed much harder.
...
This workspace doesn't ship bundled example videos. Build your own reference library instead: run `/study-creator <url>` on a few short-form videos whose editing you admire, and this skill will read each creator's `_reference/creator-library/<handle>/style-distilled.md` when you say "build this like @handle." Your `/setup` step already seeds this with the reels you pasted.
...
- `/study-creator` — study a reference creator so you can later "build this like @handle"
